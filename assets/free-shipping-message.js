class FreeShippingMeter extends HTMLElement {
    ...
    getText(cartTotalPrice, freeShipBar) {
        ...
        if (cartTotalPrice == 0) {
            ...
            this.progressBar.classList.remove('progress-hidden');
            const remainingPrice = parseInt((FreeShippingMeter.freeshipPrice * 100.0) - (cartTotalPrice * 100.0));
            console.log(FreeShippingMeter.freeshipPrice, cartTotalPrice);
            text = '<span>' + FreeShippingMeter.freeShippingText2 + ' </span><b>' + Shopify.formatMoney(remainingPrice, window.money_format) + '</b><span> ' +  FreeShippingMeter.freeShippingText3 + ' </span><span class="text">' + FreeShippingMeter.freeShippingText4 + '</span>';
